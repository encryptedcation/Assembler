...
class AsmGrader(Grader):
    ...
    def handleErrorGen(self):
        ...
        for test in tests:
            ...
            errors = os.popen(
                "bash ./run < "
                + "../automatedTesting/tests/assembly/"
                + self.ASM_ERROR_DIR
                + "/"
                + test
            ).read()
    ...
    def grade(self):
        ...
        if self.enable:
            ...
            self.printSev(
                self.HIGH,
                bcolors.OKBLUE + bcolors.BOLD + "Runing simple tests" + bcolors.ENDC,
            )
            simplePass, simpleTotal = self.handleBin(
                self.ASM_SIMPLE_DIR, self.BIN_SIMPLE_DIR
            )
            ...
            self.printSev(
                self.HIGH,
                bcolors.OKBLUE + bcolors.BOLD + "Running error tests" + bcolors.ENDC,
            )
            self.handleErrorGen()
            res = [
